...
public class UserView {
	...
	private void multiInsertUser() throws Exception{
		 ...
		 * 
		 * 1번째 userId : user100
		 * -> 사용 가능한 ID입니다 / 중복된 ID입니다. 다시 입력해주세요.
		 * 1번째 userPw : pass100
		 * 1번째 userName : 유저백
		 * -------------------------
		 * 2번째 userId : user200
		 ...
		 * 2번째 userName : 유저이백
		 * 
		 * 둘 중 하나가 실패하면 실패로 처리해야함.(rollback)
		 * 둘다 삽입 성공해야 Commit
		 * 
		 * */
		...
		// 회원가입을 여러명이 동시에 한다면 매개변수가 너무많아지므로 List 사용해서 입력받은 객체 묶기
		...
		for(int i = 0; i < input; i++) {
			...
			while(true) { // 입력할때마다 반복할 거니까 for문 아니고 while
				System.out.print("1번째 ID : ");
				userId = sc.next();
				...
				if(count == 0) { // 중복이 아닌 경우
					...
					break; 	// 중복 아닌 아이디를 입력받을때까지 반복할거라서 return 아니고 break;????
		...
		// -> 결과로 삽입된 행의 개수를 반환(int) -> 5명 등록했으면 5 반환
	...
	private void insertUser2() throws Exception {
		...
		// 입력 받은 값 3개를 한 번에 묶어서 전달할 수 있도록(따로따로 전달해도 됌. 개발자 맘임.)
		// User DTO 객체를 생성한 후 필드에 값을 세팅해보겠다.
		User user = new User();

		// setter 이용. lombok이 잘 적용됬다면 자동완성이 뜸.
		user.setUserId(userId);
		user.setUserPw(userPw);
		user.setUserName(userName);

		// 서비스 호출(INSERT) 후 결과(int 결과 행의 개수) 반환 받기
		int result = service.insertUser(user);
		// 미리 서비스에서 작성할 메서드 입력해놓고

		// 반환된 result 결과에 따라 출력할 내용 선택
		if (result > 0) {
			System.out.println("\n" + userId + "사용자가 등록되었습니다.\n");
	...
	private void updateName() throws Exception {
		...
		// 위에서 조회된 회원(userNo)의 
		// 이름 수정 서비스 호출(UPDATE) 후 결과 반환(int)받기
	...
	private void selectUser() throws Exception { 
		...
		// 사용자 번호(USER_NO) == PK(고유키) == 중복이 있을 수 없다!
	...
	private void selectName() throws Exception {
		...
		// 검색한 문자 혹은 문자열을 포함한 ID를 가진 객체가 한 개가 아닌 여러개 있을 수 있음.
		// 따라서 List(여러 객체를 배열 형태로 가지고있는 Collection)로 반환받기.
	...
	private void selectAll() throws Exception {
		...
		// dao에서 statement 혹은 preparedStatement 사용하기 위해 conn 전달

		// 결과에 따라 사용자에게 보여줄 결과 처리하기
		// 결과 1) 조회 결과가 없을 경우
		...
		// 결과 2) 조회 결과가 있을 경우
		// 향상된 for문 이용해서 userList에 있는 User 객체 하나씩 모두 출력
		for (User user : userList) { // 향상된 for문(타입 변수: 순회하는 대상(리스트/배열))
			System.out.println(user); // lombok 라이브러리 이용해 dto에서 toString 오버라이드 했기 때문에 주소값이 아닌 문자열이 출력된다.
	 ...
	 * 1. User 등록 관련 View
	 */
	private void insertUser() throws Exception { // 현재 클래스에서(swith문에서)만 호출되기 때문에
		// 컴파일러가 알아서 추론해서 접근제한자 private인 메서드로 생성해줬음
